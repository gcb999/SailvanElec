...
@interface JSBaseProductCollectionCell ()
{
    
}
@property(nonatomic,strong)JSBaseProductCollectionCellFrameModel *frameModel;

@end

@implementation JSBaseProductCollectionCell
...
-(void)JSCollectionViewController:(JSCollectionViewController *)JSCtrl collectionViewDateArr:(NSArray *)dateArr cellValue:(id)date indexPath:(NSIndexPath *)indexpath{
    ...
    JSBaseProductCollectionCellModel *model =self.frameModel.baseModel;
...
-(void)setFlashGo{
    ...
    //服务器记录时间
    NSDate *severDate=[self.frameModel.baseModel.product_flashGo_Time dateFromString];
    if (self.frameModel.baseModel.is_FlashGo) {//是闪购商品
...
#pragma mark - 商品图片坐标
    ...
    self.product_Url_ImgView.frame=self.frameModel.product_Url_ImgView_Frame;
